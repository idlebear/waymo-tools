...
DEFAULT_CACHE = "./cache/v1/motion/1_2_1/scenario/"


def parse_arguments():
    parser = argparse.ArgumentParser(description="Download Waymo data")
    parser.add_argument(
        "--cache", type=str, default=DEFAULT_CACHE, help="Cache directory to store downloaded tfrecords"
    )
...
if __name__ == "__main__":
    ...
    for index in indices:
        ...
        if not os.path.exists(os.path.join(cache_directory, tfrecord)):
            os.system(f"echo bash download_v1_motion_scenario.sh training {tfrecord}")
        else:
    ...
    for index in indices:
        ...
        if not os.path.exists(os.path.join(cache_directory, tfrecord)):
            os.system(f"bash download_v1_motion_scenario.sh validation {tfrecord}")
        else:
            print(f"File {tfrecord} exists in cache.")
